# Description:
#   CLC MRR - Returns the most recent MRR for a given account alias.
#
# Commands:
#   *@jarvis mrr <alias>* - Returns the most recent MRR for a given account alias. Will display a chart breaking down the spend per data center (Note: This data is pulled from the API and has not processed through BRM or Vantive).
...
executePowerShell = edge.func('ps', -> ###
  ...
  getMRR -alias $inputFromJS.aliasName
###
)

module.exports = (robot) ->
  # Capture the account alias being requested
  robot.respond /MRR (.*)/i, (msg) ->
    # Set the requested alias to a variable
    aliasName = msg.match[1]
    msg.send "Getting the most recent MRR for account alias #{aliasName}."
