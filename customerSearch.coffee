# Description:
#   Search for a customer account alias by company name.
#
# Commands:
#   *@jarvis customer search <search query> (example: @jarvis customer search P&G* - Search for a customer account alias by company name.
...
executePowerShell = edge.func('ps', -> ###
  ...
  . .\scripts\customerSearch.ps1
  # Edge.js passes an object to PowerShell as a variable - $inputFromJS
  # This object is built in CoffeeScript on line 28 below
  customerSearch -customerQuery $inputFromJS.customerQuery
###
)

module.exports = (robot) ->
  # Capture the account alias being requested
  robot.respond /customer search (.*)/i, (msg) ->
    # Set the requested alias to a variable
    customerQuery = msg.match[1]
    msg.send "Searching for an account alias that matches the query: *#{customerQuery}*."
